...
BrowserDetect.init();
	...
	function unhide()
	{
		if(BrowserDetect.browser == "Opera")
        	{
                	document.getElementById('panel').style.opacity = '0.0';
			//alert("This is Opera");
        	}
		else if(BrowserDetect.browser == "Explorer")
		{
			document.getElementById('panel').style.opacity = '0.0';
			document.getElementById('panel').style.filter = 'alpha(opacity=00)';
			//alert("This is Explorer");
		}
		else
		{
			document.getElementById('panel').style.opacity = '0.0; filter:alpha(opacity=00);-moz-opacity:0.0';
			//document.getElementById('panel').style = 'filter:alpha(opacity=00); opacity: 0.0;';
		...
		if(BrowserDetect.browser == "Opera")
        	{
                ...
                {
                        document.getElementById("panel").style.opacity = '1.0';
                        document.getElementById("panel").style.filter = 'alpha(opacity=100)';
                }
		else
		{
			document.getElementById("panel").style.opacity = '1.0; filter:alpha(opacity=100);-moz-opacity:1.0';
			//document.getElementById("panel").style.opacity = '1.0; filter:alpha(opacity=100);';
		}
	}
	function reposition()
	{
		...
		{
			document.getElementById("panel").style.left = 15 + 'px';
			document.getElementById("panel").style.top = 280 + 'px';
			document.getElementById("master").align = 'left';
		}
	}
	function colorStatus()
	{
		if(document.selection.pubpass.value.length < 10)
		{
			document.selection.pubpass.style.background = 'pink';
		}
		else
		{
			document.selection.pubpass.style.background = '#CCFFCC';
		}
	}
...
<body bgcolor=#330066 onLoad="reposition();">
<table align="center" valign="center" border="0" width="800" bgcolor="white" id="master">
...
<?php
		...
		if(strcmp($enablelog, "yes") == 0)
             	{
                  	$handle = fopen($basepath . "log/" . $_POST[username] . ".log", "a"); //open the global log file
                      	if($handle)
                       	{
                       		fwrite($handle, "<<".time()." ^ ".$_SERVER["REMOTE_ADDR"].":".$_SERVER["REMOTE_PORT"]." pub-authenticate.php disp");
	...
	//////////PARSE THE KEY AND PASSWORD HERE
	//echo "<p style='font-familt:Times;font-size:9px;color:red;'>";
	//echo "Key: " . $_POST[key] . "<br>";
	//echo "Selected Pass Till Now: " . $_POST[selectedpass] . "<br>";
	//echo "Public Password: " . $_POST[pubpass] . "</p>";
	/////////////////////////////////////////
	...
	if($bucket == 0) //varify the chosen password
	{
		...
		*/
		//////////////////////////////////////////////////////////////////////////////////////////////////
		$keyarray = explode("$", $_POST[key]);
                $unused_key = $keyarray[count($keyarray) - 1];
                $validseq = "";
                for($i=0; $i<strlen($_POST[pubpass]); $i++)
                {
                        if(strpos($unused_key,substr($_POST[pubpass],$i,1)) === 0) break;
                        else $validseq .= substr($_POST[pubpass],$i,1);
                }
                for($i = 0; $i < count($keyarray) - 1; $i++)
                {
                        $key_parts = explode("|", $keyarray[$i]);
                        for($j = 0; $j < strlen($validseq); $j++)
                        {
                                $char2find = substr($validseq, $j, 1);
                                if(strcmp($char2find, $key_parts[1]) == 0)
				{
					$temp .= $key_parts[0] . ";";
					$count++;
				}
			...
			if(strcmp($enablelog, "yes") == 0)
                	{
                        	...
                        	if($handle)
                        	{
                                fwrite($handle, "<<".time()." ^ ".$_SERVER["REMOTE_ADDR"].":".$_SERVER["REMOTE_PORT"]." pub-authenticate.php fail");
			...
			if(strcmp($enablelog, "yes") == 0)
                        {
                                ...
                                if($handle)
                                {
                                fwrite($handle, "<<".time()." ^ ".$_SERVER["REMOTE_ADDR"].":".$_SERVER["REMOTE_PORT"]." pub-authenticate.php succ");
                                        fwrite($handle, ">>\n");
	...
	else
	{
	 	...
	 	echo "<br><br>Please enter in the password box the characters corresponding to the images belonging to your image password set. ";
		echo "Make sure you enter the characters belonging to your password set in the beginning. Also fill in the trailing characters from ";
		echo "the <i>Unused Characters</i> displayed at the bottom in order to make your text entry in the password box at least 10 characters ";
		echo "long or until the password box turns green. <br><br><font color=red>";
		echo "If you have not chosen any image from the current image set just click on continue to move on to the next image set after filling ";
		echo "at least 10 characters from the <i>Unused Characters</i> list in the password box or until the password box turns green.";
		echo " </font>.</p>";
		//echo $_POST[key] . "<br><br>";
	 	echo "<form class=register name=selection method=post action=pub-authenticate.php>";
	 	echo "<input type=hidden name=firstname value=" . $_POST[firstname] . ">";
	 	echo "<input type=hidden name=lastname value=" . $_POST[lastname] . ">";
	 	echo "<input type=hidden name=username value=" . $_POST[username] . ">";
  	 	echo "<input type=hidden name=userfile value=" . $_POST[userfile] . ">";
		...
		echo "<input type=hidden name=usertype value=" . $_POST[usertype] . ">";
	 	...
	 	$temp = $_POST[selectedpass];
		////////////////////////////
		///here modify the processing to reflect the changes
		////////////////////////////
		/*
	 	for($i = 1; $i <=36; $i++)
	 	{
		...
		*/
		...
		echo "<input type=hidden name=nextbucket value=" . substr($_POST[nextbucket],1) . ">";
	 	...
	 	echo "Enter the corresponding text sequence [";
		echo "<input class=login type=password name=pubpass size=13 onkeyup='colorStatus();' onkeydown='colorStatus();' style='background:pink;'>] and ";
		echo " <input class=general type=submit value='Continue'> to next screen.";
         	//echo "</form>";
	 	//echo "<p class=login>Displaying Images from Bucket: " . $bucket . "</p>";
		$screenNumber = 3 - strlen($_POST[nextbucket]) + 1;
		echo "<div id='panel' style='position:absolute;width:530px;height:455px;background:#CCCC99;color:#CC3333;text-align:center;'
onMouseOver='unhide();this.style.opacity=0.0;this.filters.alpha.opacity=00' onMouseOut='hide();this.style.opacity=1;this.filters.alpha.opacity=100'>";
		echo "<br><br><br><br><br><br><br><br><br><b>Move your mouse over this panel to unhide the images.</b><br><br><br>";
		echo "<b>This is image grid #" . $screenNumber . " of 3 IBA image grids.</b>";
		echo "</div>";
		$string = str_shuffle("abcdefghjkmnpqrstuvwxyzABCDEFGHIJKLMNPQRSTUVWXYZ23456789");
		$key = "";
	 	...
	 	for($i = 1; $i<=36; $i++)
	 	{
			if($array[$i] < 10)
			{
				echo "<td align=center style='font-family:Times;font-size:11px;color:navy;'>";
				echo "<img name=img" . $bucket . "0" . $array[$i] . " src=images/bucket" . $bucket . "/sharp/0" . $array[$i] . 
				".jpg onMouseOver=change(" . $array[$i] . ","  . $bucket . "); onMouseOut=reset(" . $array[$i] . "," . 
				$bucket . "); width=80 height=53 border=0  style='border-color:red;'>";
				echo "<br><b>" . substr($string, $i - 1, 1) . "</b>";
				$key .= "images/bucket" . $bucket . "/sharp/0" . $array[$i] . ".jpg|" . substr($string, $i - 1, 1) . "$";
				
			}
			else
			{
				echo "<td align=center style='font-family:Times;font-size:11px;color:navy;'>";
			 	echo "<img name=img" . $bucket . $array[$i] . " src=images/bucket" . $bucket . "/sharp/" . $array[$i] . 
				".jpg onMouseOver=change(" . $array[$i] . "," . $bucket . "); onMouseOut=reset(" . $array[$i] . "," . 
				$bucket . "); width=80 height=53 border=0 style='border-color:red;'>";
				echo "<br><b>" . substr($string, $i - 1, 1) . "</b>";
				$key .= "images/bucket" . $bucket . "/sharp/" . $array[$i] . ".jpg|" . substr($string, $i - 1, 1) . "$";
			}
			if(($i % 6 == 0) && ($i != 36)) echo "<tr>";
	 	}
		echo "<tr><td colspan=6 align=center style='font-family:Times;font-size:14px;color:navy;'>";
		echo "<u>Unused Characters</u>: <b>" . substr($string, 36, strlen($string) - 35) . "</b>";
		$key .= substr($string, 36, strlen($string) - 35);
	 	echo "</table>";
